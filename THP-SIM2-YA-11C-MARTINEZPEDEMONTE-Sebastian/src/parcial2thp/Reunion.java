...
public class Reunion {
	...
	public Resultado crearPropuesta(String nombre, Dia dia, Momento momento) {
		...
		if (amigo == null) {
			...
			resultado = Resultado.OPERACION_OK;
			// System.out.println(resultado);
			System.out.println("Agregando propuesta " + dia + "(" + momento + ")" + ". Propuesta por " + nombre);
	...
	public Resultado unirAPropuesta(String nombre, Dia dia, Momento momento) {
		...
		if (amigo == null) {
			resultado = Resultado.AMIGO_NO_EXISTENTE;
			System.out.println(nombre + " se quiere agregar al " + dia + "(" + momento + ")");
			System.out.println(resultado);
	...
	//buscarAmigo con Stream
	private Amigo buscarAmigo(String nombre) {
		Amigo amigoBuscar = null;
		try {
			amigoBuscar = amigos.stream().
					filter(a -> a.getNombre().equals(nombre)).findFirst().get();
	...
	/*
	 private Amigo buscarAmigo(String nombre) { int i = 0; Amigo amigoBuscar =
	 null; while(amigos.size() > i && amigoBuscar == null) {
	 if(amigos.get(i).getNombre().equals(nombre)) { amigoBuscar= amigos.get(i);
	 }else { i++; } } return amigoBuscar; }
	 
	
		  private Propuesta buscarPropuesta(Dia dia, Momento momento) { int i = 0;
		  Propuesta propuestaBuscar = null; while(propuestas.size() > i &&
		 propuestaBuscar == null) { if(propuestas.get(i).getDia().equals(dia) &&
		  propuestas.get(i).getMomento().equals(momento)) { propuestaBuscar=
		  propuestas.get(i); }else { i++; } } return propuestaBuscar; }
		 */
	private Propuesta buscarPropuesta(Dia dia, Momento momento) {
		Propuesta propuestaBuscar = null;
		try {
			propuestaBuscar =	propuestas.stream()
				.filter(p -> p.getDia().equals(dia)  && p.getMomento().equals(momento)  )
				.findFirst()
				.get();
		} catch (RuntimeException e) {
	...
	public Resultado cambiarDePropuesta(String nombre, Dia diaOriginal, Momento momentoOriginal, Dia diaActualizado,
			Momento momentoActualizado) {
		Resultado resultado = null;
		if (buscarPropuesta(diaOriginal, momentoOriginal) == null
				|| buscarPropuesta(diaActualizado, momentoActualizado) == null) {
			resultado = Resultado.PROPUESTA_NO_EXISTENTE;
			System.out.println(resultado);
		...
		System.out.println(nombre + " se pasa del " + diaOriginal + " (" + momentoOriginal + ") al " + diaActualizado
				+ " (" + momentoActualizado + ")");
		return resultado;
	}
	...
	public Resultado bajarDePropuesta(String nombre, Dia dia, Momento momento) {
		...
		if (amigo == null) {
			...
			resultado = Resultado.PROPUESTA_NO_EXISTENTE;
			System.out.println(nombre + " se borra del " + dia + "(" + momento + ")");
			System.out.println(resultado);
		} else {
	...
	/*
	 * yo hice un foreach de amigos anotados que adentro tenía un foreach de
	 * propuestas entonces para cada amigo recorres todas las propuestas, entonces
	 * si el amigo no estaba en ninguna propuesta lo agregue a un array de amigos no
	 * anotados, y retorne ese array
	 */
	...
	public void listarPropuestas() {
		...
		/*
		 * for(Propuesta propuesta : propuestas) { propuesta.listarAmigos(); }
		 */
